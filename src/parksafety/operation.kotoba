(ns parksafety.operation
  "OperationActor -- one amusement-park/ride-safety operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (ParkOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Ride Safety Governor (:govern)
  ...
  One graph run = one amusement-park/ride-safety operation (intake ->
  ...
  decision to a human operator (the licensed ride operator/inspector).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:ride/reopen` ALWAYS reaches this node when the governor
  is clean -- see `parksafety.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [parksafety.parkopsllm :as parkopsllm]
...
(defn build
  ...
  `parksafety.store/Store`).
  opts:
    :advisor      -- a `parksafety.parkopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (parkopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ParkOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (parkopsllm/-advise advisor store request)]
            {:proposal p :audit [(parkopsllm/trace request p)]})))

      ;; Ride Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
